...
def show_all(functions, x=None, fitted_function=True, other_func=None,
                 display=True, tolerance=0.001, title=None, axes=None,
                 layout="auto", writer=None, step=None, colors="#1f77b4"):
    logger = ActivationLogger("ActivationLogger")
    if axes is None:
            if layout == "auto":
                total = len(functions)
...
def save_all_inputs(functions, *args, **kwargs):
    for func in functions:
        func.save_inputs(*args, **kwargs)


def change_category(functions, new_category):
    all_functions = []
...
def get_activations(network, top_level=False):
    ...
    for name, object in network.named_children():
        if isinstance(object, ActivationModule):
            ...
            found_instances[name] = _process_recursive(found_instances, name, object)
...
def _process_recursive(original_dict, recName, recObject):
    ...
    for name, object in recObject.named_children():
        if isinstance(object, ActivationModule):
            ...
            original_dict[wholeName] = _process_recursive(original_dict, name, object)
